...
const (
	...
	partialSucess    = `{"partial success":"able to upload some data to amoss_mhealth files",
...
func (uh UploadHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	switch currentParticipant.Study {
	case "vismet":
		bucket = currentParticipant.Study
	case "cfd-sleep-study-test":
		bucket = currentParticipant.Study
	case "cfd-sleep-study":
		bucket = currentParticipant.Study
	case "cfd-classroom-audio":
		bucket = currentParticipant.Study
	case "schizo":
		bucket = "schizo-eye-tracking"
	default:
	...
	for i, f := range files {
		...
		if !fullUpload {
			...
			log.Printf("This is the result of the upload: %s\n{Key: %s, Success: full}\n", uploadResult.GoString(), key)
			w.Header().Add("Strict-Transport-Security", "max-age=63072000; includeSubDomains")
			w.Write([]byte("{\"success\": \"you have completed upload to amoss_mhealth\"}"))
		}
	}
}
...
func (uh UploadMoyoHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	if accessTokenDB == bearerToken {
		...
		if !fullUpload {
			...
			w.Header().Add("Strict-Transport-Security", "max-age=63072000; includeSubDomains")
			w.Write([]byte("{\"success\": \"you have completed upload to amoss_mhealth/moyo\"}"))
		}
	} else {
...
func (uh UploadUTSWHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	for i, f := range files {
		...
		if !fullUpload {
			...
			log.Printf("This is the result of the upload: %s\n{Key: %s, Success: full}\n", uploadResult.GoString(), key)
			w.Header().Add("Strict-Transport-Security", "max-age=63072000; includeSubDomains")
			w.Write([]byte("{\"success\": \"you have completed upload to amoss_mhealth/utsw\"}"))
		}
	}
}
...
func (uh UploadHFHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	for i, f := range files {
		...
		if !fullUpload {
			...
			log.Printf("This is the result of the upload: %s\n{Key: %s, Success: full}\n", uploadResult.GoString(), key)
			w.Write([]byte("{\"success\": \"you have completed upload to amoss_mhealth/hf\"}"))
		}
	}
}
