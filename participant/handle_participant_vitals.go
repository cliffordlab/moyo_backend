...
func updateDBIsVerified(key string, id string, creationTime string, writer http.ResponseWriter) {
	...
	defer stmt.Close()
	//`-- UPDATE bp_readings SET (sdp, bdp, pulse) VALUES ($1, $2, $3) where participant_id=$4 and created_at=$5 and s3_path=$6;`
	...
	writer.WriteHeader(http.StatusOK)
	writer.Write([]byte("{\"success\": \"you have completed update to amoss_mhealth/moyo-mom-emory/\"}"))
}
...
func renameS3Object(u UnverifiedBPFileHandler, s3Key string) {
	...
	// Copy the item
	_, err := u.Svc.CopyObject(&s3.CopyObjectInput{Bucket: aws.String("amoss-mhealth"), CopySource: aws.String("amoss-mhealth/" + s3Key), Key: aws.String(s3Key + "_old")})
	if err != nil {
	...
	err = u.Svc.WaitUntilObjectExists(&s3.HeadObjectInput{Bucket: aws.String("amoss-mhealth"), Key: aws.String(s3Key + "_old")})
	if err != nil {
		fmt.Printf("Item %q copy unsuccessful: " + s3Key)
		return
	}
	...
	_, err = u.Svc.DeleteObject(&s3.DeleteObjectInput{Bucket: aws.String("amoss-mhealth"), Key: aws.String(s3Key)})
	if err != nil {
	...
	err = u.Svc.WaitUntilObjectNotExists(&s3.HeadObjectInput{Bucket: aws.String("amoss-mhealth"), Key: aws.String(s3Key)})
	if err != nil {
		fmt.Printf("Item %q delete unsucessful: " + s3Key)
		return
	}
...
func GetS3PreSignedUrl(key string, u UnverifiedBPFileHandler) string {
	...
	req, _ := u.Svc.GetObjectRequest(&s3.GetObjectInput{
		Bucket: aws.String("amoss-mhealth"),
